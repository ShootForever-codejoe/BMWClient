...
object GrimVelocityFull : Choice("Full") {
    ...
    private val packetEventHandler = sequenceHandler<PacketEvent> { event ->
        ...
        if (((packet is EntityVelocityUpdateS2CPacket && packet.entityId == player.id)
            ...
            if (player.activeItem.useAction != UseAction.EAT
                ...
                delay = true
                needClick = true
                if (debug) notifyAsMessage("[GrimVelocity] Cancel Velocity")
            } else {
                if (debug) notifyAsMessage("[GrimVelocity] Receive Velocity")
            }
            canCancel = false
        }
    }
    ...
    private val playerTickEventHandler = handler<PlayerTickEvent> { event ->
        if (needClick) {
            val hitResultYaw = player.yaw - (0.01f..0.1f).random()
            val hitResultPitch = 90f - (0.01f..0.1f).random()
            val hitResult = raycast(rotation = Rotation(hitResultYaw, hitResultPitch))
            val pos = hitResult.blockPos.offset(hitResult.side)
            ...
            if (pos.equals(player.blockPos) && !shouldSkip) {
                ...
                if (RotationManager.serverRotation.yaw != hitResultYaw || RotationManager.serverRotation.pitch != hitResultPitch) {
                    network.sendPacket(
    ...
    private val tickHandler = tickHandler {
        ...
        waitForUpdate = false
        needClick = false
        if (debug) notifyAsMessage("[GrimVelocity] Max Stuck Ticks")
    }
